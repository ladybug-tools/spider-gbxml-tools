...
POP.getIntersectedVertexBufferGeometry = function( intersected, intersects ) {
	...
	POP.particle.scale.x = POP.particle.scale.y = 0.01 * THRU.radius;
...
POP.getIntersectedDataHtml = function( intersected, intersects ) {
	...
	if ( adjSpaces ) {
		...
		storey = storeys.find( item => item.id === space1.buildingStoreyIdRef );
	...
	`
		...
		<p><b>Navigating</b></p>
		<i>Click buttons twice to see how they toggle</i>
...
POP.toggleStoreyVisible = function( button, storeyId ) {
	...
	const storeyJson = storeys.find( item => item.id === storeyId );
	//console.log( 'spaceJson', spaceJson );
