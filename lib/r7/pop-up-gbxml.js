...
const POP = { "release": "SGV Pop-Up R7.4" };
...
POP.getMenuHtmlPopUp = function() {
	...
	`
		...
		<div>
			<p style=text-align:right; > <a href="https://github.com/ladybug-tools/spider-gbxml-tools/tree/master/cookbook/spider-gbxml-viewer-pop-up" >${ POP.release }</a></p>
		</div>
...
POP.getIntersectedDataHtml = function( intersected, intersects ) {
	...
	`
		...
		<p>
			<button id=POPbutSurfaceFocus onclick=POP.toggleSurfaceFocus(); >surface: ${ surfaceJson.id }</button>
			<button id=POPbutSurfaceVisible onclick=POP.toggleSurfaceVisible(); ><img src="https://www.ladybug.tools/spider-gbxml-tools/assets/eye.png" height=12 ></button>
			<button onclick=POP.setSurfaceZoom(); title="Zoom surface" >⌕</button>
		</p>
		<p>
			${ adjSpaceButtons }
		</p>
		...
		<details>
			<summary>Current design reference and issues</summary>

			<p>Identifying according to <a href="http://gbxml.org/schema_doc/6.01/GreenBuildingXML_Ver6.01.html" target="_blank">gbXML Schema</a></p>
		...
		if ( typeof( obj[ key ] ) === 'object' ) {
			...
			`
				<div>
					...
					<span class="attributeValue" >${ isNaN( Number( obj[ key ] ) ) ? obj[ key ] : Number( obj[ key ] ).toLocaleString() }</span>
				</div>
			`;
