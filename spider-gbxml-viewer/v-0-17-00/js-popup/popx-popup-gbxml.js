...
POPX.getIntersectedDataHtml = function() {
	...
	const surfaceIndexed = GBX.surfaces[ index ];
	//console.log( 'surfaceIndexed', surfaceIndexed );

	//const surfaceText = surfaceIndexed.slice( surfaceIndexed.indexOf( '"<' ) + 1 );

	const surfaceXml = POPX.parser.parseFromString( surfaceIndexed, "application/xml").documentElement;
